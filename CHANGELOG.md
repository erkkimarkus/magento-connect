...
- The abandoned-cart job now completes: since the guest-cart widening its quote query carried an ambiguous column reference that made MySQL reject the whole SELECT, so the job failed on every run and no reminder was ever sent, on any install. Fixed by qualifying the filter column.
