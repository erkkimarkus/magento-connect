...
- Full multilingual admin UI: when store views speak more than one language, the Connection panel (wizard step 1 / Settings > Connection) offers four routing-mode choice cards; "Per-language Smaily accounts" mode manages one credential block per language (each with its own Test Connection) plus a default-fallback account picker, and both per-language modes add a per-language workflow mapping editor to the Automations panel with live per-account workflow lists and a default-fallback row per trigger. All sections adapt live to the selected mode — nothing saves until you say so. A mapped workflow always fires through the Smaily account its mapping row names (fallback rows included), matching the WooCommerce plugin's routing.
...
- Multi-website support: installs with more than one Magento website get an explicit website selector on Settings (and a website-picker step in the Setup Wizard), so each website keeps its own Smaily connection, subscriber sync and automation settings. Single-website installs are unaffected.
